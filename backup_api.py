...
def incremental_backup(parameters):
    ...
    dumpdata_apps = (
        'django_dialog_engine_builder.InteractionCard',
    )

    prefix = 'django_dialog_engine_builder_backup_' + settings.ALLOWED_HOSTS[0]
    ...
    for app in dumpdata_apps:
        logger.info('[django_dialog_engine_builder] Backing up %s...', app)
        sys.stdout.flush()
