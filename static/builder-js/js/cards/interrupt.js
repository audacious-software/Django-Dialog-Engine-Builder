    ...
    class InterruptNode extends Node {
        ...
        editBody() {
            var body = '<div class="mdc-layout-grid__cell mdc-layout-grid__cell--span-12">' +
                       '  <label class="mdc-text-field mdc-text-field--outlined mdc-text-field--textarea" id="' + this.cardId + '_patterns_field" style="width: 100%">' +
                       '    <span class="mdc-notched-outline">' +
                       '      <span class="mdc-notched-outline__leading"></span>' +
                       '      <div class="mdc-notched-outline__notch">' + 
                       '        <label for="' + this.cardId + '_patterns_value" class="mdc-floating-label">Matching Patterns</label>' + 
                       '      </div>' + 
                       '      <span class="mdc-notched-outline__trailing"></span>' +
                       '    </span>' +
                       '    <span class="mdc-text-field__resizer">' +
                       '      <textarea class="mdc-text-field__input" rows="4" style="width: 100%" id="' + this.cardId + '_patterns_value"></textarea>' +
                       '    </span>' +
                       '  </label>' +
                       '<div class="mdc-layout-grid__cell mdc-layout-grid__cell--span-7 mdc-typography--caption" style="padding-top: 8px;">' +
                       '  Please enter one pattern per line.' +
                       '</div>' +
        ...
        viewBody() {
			var body = '<div class="mdc-typography--body1" style="margin: 16px;">Interrupts when matching:</div>';
			body +=    '<ul>';
        ...
        initialize() {
			...
			$('#' + this.cardId + '_patterns_value').on("change keyup paste", function() {
				var value = $('#' + me.cardId + '_patterns_value').val();
